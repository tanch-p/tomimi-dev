<script lang="ts">
	...
	let { enemy, row, language, specialMods, mapConfig }: Props = $props();
	let traits = $derived(getEnemySkills(enemy, enemy.traits, row, $specialMods, 'trait'));
	let specialList = $derived(
		getEnemySkills(enemy, enemy.forms[row].special, row, $specialMods, 'special')
	);
	let statusImmuneList = $derived(
		getStatusImmune(enemy, enemy.forms[row].status_immune, $specialMods)
	);
</script>
