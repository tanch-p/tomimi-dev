...
const SEARCH_IN_BLACKBOARD_SET = new Set(SEARCH_IN_BLACKBOARD);
...
function someCheck(itemArr, selectedOptions) {
	...
	selectedOptions = selectedOptions.filter(
		(val) => !["condition_none", "others"].includes(val),
	);
	const list = itemArr ?? [];
	return (
		othersCheck ||
		noneCheck ||
		list.some((val) => selectedOptions.includes(val))
	);
}
	...
	char.uniequip
		...
		.forEach((equip) => {
			equip.combatData.blackboard.forEach((ele) => {
				if (
					ele.key === key &&
					functions.every((fn) => fn(ele, equip.combatData.tags))
				) {
					values.push(ele[subKey] ?? defaultValue);
				}
			});
		...
		char.uniequip
			...
			.reduce((acc, curr) => {
				...
				curr.combatData.blackboard.forEach((ele) => {
					if (
						ele.key === key &&
						functions.every((fn) => fn(ele, curr.combatData.tags))
					) {
						const val = ele[subKey] ?? defaultValue;
						if (val > value) value = val;
					}
...
export const getModuleTrait = (trait, module, stage: number) => {
	...
	for (const part of module.combatData.phases[stage].parts) {
		if (
			!part.isToken &&
			(part.target.includes("TRAIT") || part.target === "DISPLAY")
		) {
...
export const getModuleUpdatedRange = (
	rangeId,
	module,
	stage,
	talentIndex = -1,
...
export const getSkillRangeId = (skill, char, moduleIndex, moduleStage) => {
	if (skill.levels?.[0]?.rangeId?.includes("get_talent")) {
		...
		return getModuleUpdatedRange(
			talentRangeId,
			char.uniequip[moduleIndex],
			moduleStage,
			index,
		);
	}
...
export const getFullCharStat = (statKey, char, moduleStat, potStat) => {
	...
	if (statKey === "aspd") {
		return (
			Math.round(
				(stat /
					((100 + (char?.favorData?.[statKey] ?? 0) + moduleStat + potStat) /
						100)) *
					100,
			) / 100
		);
...
export const getPrioritySortValues = (char, sortOptions, secFilters) => {
	...
	return uniqueValues
		.map((value) => {
			...
			if (
				["damage_scale", "ally_damage_scale", "ally_heal_scale"].includes(
					key,
				) &&
				value < 4.9
			) {
...
export const getCategory = (value: string): "blackboard" | "tags" => {
	return SEARCH_IN_BLACKBOARD_SET.has(value) ? "blackboard" : "tags";
};
			...
			if (key === "deployable_tile") {
				if (
					!searchItem.options.some(
						(val) =>
							char.position === val || char.tags.includes("position_all"),
					)
				) {
					let equipIndex = char.uniequip
						.filter((equip) => equip.combatData)
						.findIndex((equip) =>
							equip.combatData.tags.includes("position_all"),
						);
				...
				char.skills.forEach((skill, i) => {
					if (
						(options.includes(skill.spType) ||
							options.includes(skill.skillType)) &&
						options.every((option) =>
							secFiltersFunctions[type][option].every((fn) =>
								fn(skill, skill.tags),
							),
						)
					) {
				...
				if (!found) {
					...
					if (equipIndex !== -1) {
						char.activeModuleIndex = equipIndex + 1;
						const moduleTalent = getModuleRelatedTalentIndex(
							char,
							equipIndex + 1,
						);
				...
				if (
					char.blackboard.some(
						(item) =>
							item.key === key &&
							secFiltersFunctions?.[key]?.every((fn) => fn(item, char.tags)),
					)
				) {
					found = true;
				}
				...
				char.talents.forEach((talent, i) => {
					if (
						talent.blackboard.some(
							(item) =>
								item.key === key &&
								secFiltersFunctions?.[key]?.every((fn) =>
									fn(item, talent.tags),
								),
						)
					) {
				...
				if (blockCntSubFilters.includes("skill_active")) {
					...
					if (skill) {
						const skillIndex = char.skills.findIndex(
					...
					char.tokens.forEach((token, i) => {
						if (
							...
							token.blackboard.some(
								(item) =>
									item.key === "blockCnt" && options.includes(item.value),
							)
						) {
							found = true;
							char.activeTokens.push(i);
	...
	return (char) => {
		...
		for (const remainder of skillRemainder) {
			...
			remainder.forEach((searchItem) => {
				if (
					searchItem.type === "blockCnt" &&
					!searchItem.options.includes(char.stats.blockCnt)
				) {
		...
		for (const remainder of finalRemainder) {
			...
			if (equipIndex !== -1) {
				...
				if (
					!finalRemainder.some(
						(remainder) =>
							remainder.length === 1 && remainder[0].key === "blockCnt",
					)
				) {
					const moduleTalent = getModuleRelatedTalentIndex(
						char,
	...
	const returnArr = optionsList.map(([subKey, options]) => {
		...
		return {
			...
			options: options.sort(
				(a, b) => getConditionWeights(a) - getConditionWeights(b),
			),
			type: "options",
		};
	});
		...
		if (type === "compare") {
			switch (subKey) {
				case "spType":
					...
					fn = (skill) => {
						const { initSp, spCost } =
							skill.levels[skill.levels.length - 1].spData;
