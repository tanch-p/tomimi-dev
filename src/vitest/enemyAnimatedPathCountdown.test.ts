...
describe('animated path countdown fade', () => {
	it('fades over five seconds and then removes only the animated path countdown', () => {
		...
		enemy.updateAnimatedPathCountdowns(2.5);

		expect(setCountdownOpacity).toHaveBeenCalledWith(42, 0.5);
		expect(removeCountdown).not.toHaveBeenCalled();
		expect(enemy.animatedPathCountdowns).toEqual([{ id: 42, elapsed: 2.5 }]);
